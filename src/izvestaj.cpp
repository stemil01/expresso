...
void izvestaj::unesiTrosakNabavke() {
  bool ok;
  double trosakNabavke =
      QInputDialog::getDouble(nullptr, "Trošak nabavke",
                              "Unesite trošak nabavke:", 0, 0, 1000000, 2, &ok);
  if (!ok) {
    qDebug() << "Unos troška nabavke otkazan";
    return;
  }

  QString datumString = QInputDialog::getText(
      nullptr, "Datum nabavke",
      "Unesite datum nabavke (format: yyyy-MM-dd):", QLineEdit::Normal,
  ...
  if (!datum.isValid()) {
    ...
    QMessageBox::warning(nullptr, "Greška",
                         "Uneli ste neispravan datum. Pokušajte ponovo.");
    return;
  }
...
void izvestaj::cistPrihodMesec(const QDate &datum) {
  ...
  bool ok;
  int brojRadnika = QInputDialog::getInt(
      nullptr, "Broj radnika", "Unesite broj radnika:", 0, 0, 100, 1, &ok);
  if (!ok) {
    qDebug() << "Unos broja radnika otkazan"; // Ako je unos otkazan, prekini
                                              // operaciju
  }

  double ukupnePlate = 0;
  for (int i = 1; i <= brojRadnika; ++i) {
    double plata = QInputDialog::getDouble(
        nullptr, "Plata radnika",
        QString("Unesite platu za radnika %1:").arg(i), 0, 0, 1000000, 2, &ok);
    if (!ok) {
  ...
  // Unos stope PDV-a
  double stopaPDV = QInputDialog::getDouble(
      nullptr, "PDV", "Unesite stopu PDV-a (%):", 20.0, 0, 100, 2, &ok);
  if (!ok) {
  ...
  QMessageBox::information(
      nullptr, "Čist prihod",
      QString("Čist prihod bez PDV-a: %1 RSD\nČist prihod sa PDV-om: %2 RSD")
          .arg(cistPrihod)
          .arg(cistPrihodSaPDV));
}
