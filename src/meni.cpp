...
Meni::Meni() { _binarySerializer = new BinarySerializer("data/meni"); }
...
void Meni::fromVariant(const QVariant &variant) {
  ...
  for (const auto &variantKategorija : variantKategorije) {
    ...
    _kategorije[nazivKategorije]->fromVariant(
        mapKategorija.value("kategorija"));
  }
  /*
      qDeleteAll(_meni);
      _meni.clear();
...
void Meni::executeLoad() { _binarySerializer->load(*this, "meni.bin"); }

void Meni::executeSave() const { _binarySerializer->save(*this, "meni.bin"); }

QMap<QString, Kategorija *> Meni::getKategorije() { return _kategorije; }
