...
Naruci::Naruci(QWidget *parent, Porudzbina *porudzbina,
               unesiartikle *_unesiArtikle, Meni *meni)
    : QDialog(parent), ui(new Ui::Naruci), p(porudzbina),
      unesiArtikle(_unesiArtikle), _meni(meni) {
  ui->setupUi(this);

  connect(ui->pbAddItemOrderDialog, &QPushButton::clicked, this,
          &Naruci::onPbAddItemOrderDialogClicked);
  connect(ui->pbReceiptOrderDialog, &QPushButton::clicked, this,
          &Naruci::onPbReceiptOrderDialogClicked);
  connect(ui->pbDeleteOrderDialog, &QPushButton::clicked, this,
          &Naruci::deleteSelectedRow);
  connect(ui->pbBackOrderDialog, &QPushButton::clicked, this, &QDialog::accept);
  connect(ui->cbTypeOrderDialog, &QComboBox::currentTextChanged, this,
          &Naruci::comboBoxTextChanged);
  connect(ui->pbSplitBillOrderDialog, &QPushButton::clicked, this,
          &Naruci::onPbSplitBillOrderDialogClicked);
...
void Naruci::onPbAddItemOrderDialogClicked() {
  ...
  for (const auto &item : selectedItems) {
    ...
    Artikl *artikl =
        (_meni->getKategorije())[kategorija]->getArtiklByNaziv(naziv);
    int promena;
...
void Naruci::addItemInTW(QTableWidget *tw, Artikl *artikl) {
  ...
  p->getSto()->setStatus(Sto::OCCUPIED);
}
...
void Naruci::onPbReceiptOrderDialogClicked() {
  ...
  ui->twOrderOrderDialog->model()->removeRows(
      0, ui->twOrderOrderDialog->rowCount());
  p->obrisiArtikle();
  p->getSto()->setStatus(Sto::AVAILABLE);
  // delete p;
}
...
void Naruci::onPbSplitBillOrderDialogClicked() {
  ...
  if (selectedRanges.isEmpty()) {
    QMessageBox::warning(this, "Warning",
                         "If you want to split the bill, please select the "
                         "desired items from the order.");
    return;
  }
