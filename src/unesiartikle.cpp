...
unesiartikle::unesiartikle(QWidget *parent)
    : QWidget(parent), ui(new Ui::unesiartikle) {
  ui->setupUi(this);
  connect(ui->pushButton, &QPushButton::clicked, this,
          &unesiartikle::onSubmitClicked);
}

unesiartikle::~unesiartikle() { delete ui; }
...
void unesiartikle::setSkladisteArtikala(
    const std::vector<Artikl> &newSkladisteArtikala) {
...
void unesiartikle::ispisiPoKategorijiListWidget(
    QListWidget *lw, const QString &odabranaKategorija) {
  if (lw) {
...
double unesiartikle::cenaArtikla(const QString &_kategorija,
                                 const QString &_naziv) {
  for (const auto &artikl : skladisteArtikala) {
